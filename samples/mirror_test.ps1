...
$ifstestParameters = @(
	"-t", "FileNameLengthTest",			   # reason: buffer overflow in mirror. Issue #351
	"-t", "EndOfFileInformationTest",      # reason: IFSTest crashes 😲. Issue #x
	"-t", "NotificationSecurityTest",      # reason: IFSTest hangs 😞. Issue #x
	"-t", "NotificationCleanupAttribTest"  # reason: bothersome to wait for timeout. Issue #x
	"/v",                                  # verbose output
	"/u", $ifstest_user,
	"/U", $ifstest_pass
)
